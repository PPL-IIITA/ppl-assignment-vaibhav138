...
#include "Girl.h"
using namespace std;
class Gifting/*! \brief To perform exchanges of gifts
    *
    *class with functions to perform exchage of gifts
    */{
	private :
		/**Array containg objects of boys*/
		Boy b[100];
		/**Array containing objects of girls*/
		Girl g[100];
		/**Array containg luxury gifts*/
		Luxury l[300];
		/**Array containg utility gifts*/
		Utility u[300];
		/**Array containing essential gifts*/
		Essential e[300];
		/**Number of boys*/
		int nb;
		/**Number of girls*/
		int ng;
		void sort_l(int nl)
		{/**Sort luxury items acc. to price*/
			int i,j;
			Luxury temp;
			for(i = 0;i < nl;i++){
				for(j = i+1;j < nl;j++){
		...
		void sort_u(int nu)
		{/**Sort utility items acc. to price*/
			int i,j;
			Utility temp;
			for(i = 0;i < nu;i++){
				for(j = i+1;j < nu;j++){
		...
		void sort_e(int ne)
		{/**Sort essential items acc. to price*/
			int i,j;
			Essential temp;
			for(i = 0;i < ne;i++){
				for(j = i+1;j < ne;j++){
		...
		{/**Find object in boys array with given name*/
			int i;
			for(i = 0;i < nb;i++){
				if(b[i].get_name() == bf){
					return b[i];
		...
		{/**Find object in girls array with given name*/
			int i;
			for(i = 0;i < ng;i++){
				if(g[i].get_name() == gf){
					return g[i];
		...
		{/**Scan boys and girls from data files and store them in respective arrays*/
